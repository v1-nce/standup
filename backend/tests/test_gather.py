...
from gather import candidates, gather, scope
from models import Commit, FileFacts, Index, Scope
...
async def test_the_model_is_told_the_date_but_never_the_candidates(index):
    ...
    await gather(client, index, "standup tomorrow", today=TODAY, slide_budget=3)
...
async def test_gather_returns_the_settled_scope_beside_what_it_selected(index):
    client = StubClient(Scope(since=THURSDAY, keywords=["auth"], slide_budget=2))
    settled, chosen = await gather(client, index, "the auth thing", today=TODAY, slide_budget=2)
    assert settled.keywords == ["auth"]
    assert [c.id for c in chosen] == ["src/auth.py"]
