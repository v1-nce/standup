...
from llm.gemini_client import GeminiClient, _reason, _spoken
...
def test_nothing_configured_names_both_keys(unconfigured):
    assert llm.active_provider() is None
    with pytest.raises(NotConfigured, match="ANTHROPIC_API_KEY or GEMINI_API_KEY"):
        llm.from_settings()
...
def test_an_unreadable_error_body_still_reports_the_status():
    ...
    assert "HTTP 500" in _reason(error)
